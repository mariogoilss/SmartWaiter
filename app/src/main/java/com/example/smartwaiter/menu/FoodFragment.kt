 ...
 */
private lateinit var recyclerViewFoodListOrg: RecyclerView
private val adapterMenuOrgRV: AdapterMenuOrgRV = AdapterMenuOrgRV()
var arrayFoodListOrg = ArrayList<MenuItem>()//<-- Declaramos el arrayList a devolver
...
class FoodFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        if (arrayFoodListOrg.isEmpty()) {
            load(view)
        } else {
            recyclerViewFoodListOrg = view.findViewById<RecyclerView>(R.id.rvMenuFoodOrg)
            recyclerViewFoodListOrg.setHasFixedSize(true)
            recyclerViewFoodListOrg.layoutManager = LinearLayoutManager(context!!)
            adapterMenuOrgRV.AdapterMenuOrgRV(arrayFoodListOrg, context!!, true)
            recyclerViewFoodListOrg.adapter = adapterMenuOrgRV
        }
        return view
    }
    ...
    private fun load(view: View) {
        db.collection("organizations").document(idOrganization).get().addOnSuccessListener {
            ...
            arrayToHash =
                it.get("orgFoodList") as ArrayList<MenuItem> //<-- Pillamos tabla hash BBDD
    ...
    private fun getOfBBDD(id: String, menuItem: MenuItem) {
        ...
        db.collection("organizations").document(id).get().addOnSuccessListener {
            ...
            arrayFoodListOrg.add(menuItem)
            adapterMenuOrgRV.notifyItemInserted(arrayFoodListOrg.size)
            adapterMenuOrgRV.notifyItemRangeChanged(arrayFoodListOrg.size, arrayFoodListOrg.size)
